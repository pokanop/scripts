...
aikit knows 34 agents, but not all of them can be pointed at a third-party gateway.
...
```bash
aikit gateway coverage        # the full 34-row matrix: agent, detected?, state, how/why
```
...
Read-only. Prints the full **34-row coverage matrix** — for every agent aikit knows:
